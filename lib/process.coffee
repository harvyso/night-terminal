...
filteredEnvironment = do ->
  ...
  env.TERM_PROGRAM = 'night-terminal'
  return env
...
module.exports = (pwd, shell, args, env, options={}) ->
  ...
  emitTitle = _.throttle ->
    emit('night-terminal:title', ptyProcess.process)
  , 500, true

  ptyProcess.on 'data', (data) ->
    emit('night-terminal:data', data)
    emitTitle()

  ptyProcess.on 'exit', ->
    emit('night-terminal:exit')
    callback()
  ...
  process.on 'message', ({event, cols, rows, text}={}) ->
    switch event
      ...
      when 'pty' then emit('night-terminal:pty', ptyProcess.pty)
