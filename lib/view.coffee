...
class PlatformIOTerminalView extends View
  ...
  @content: ->
    @div class: 'night-terminal terminal-view', outlet: 'platformIOTerminalView', =>
  ...
  initialize: (@id, @pwd, @statusIcon, @statusBar, @shell, @args=[], @env={}, @autoRun=[]) ->
    ...
    @prevHeight = atom.config.get('night-terminal.style.defaultPanelHeight')
    ...
    @setAnimationSpeed()
    @subscriptions.add atom.config.onDidChange 'night-terminal.style.animationSpeed', @setAnimationSpeed

    override = (event) ->
      return if event.originalEvent.dataTransfer.getData('night-terminal') is 'true'
    ...
    @xterm.on 'mouseup', (event) =>
      if event.which != 3
        ...
        if atom.config.get('night-terminal.toggles.selectToCopy') and text
    ...
    if /zsh|bash/.test(@shell) and @args.indexOf('--login') == -1 and Pty.platform isnt 'win32' and atom.config.get('night-terminal.toggles.loginShell')
      @args.unshift '--login'
  ...
  setAnimationSpeed: =>
    @animationSpeed = atom.config.get('night-terminal.style.animationSpeed')
  ...
  displayTerminal: ->
    ...
    @terminal = new Terminal {
      cursorBlink     : false
      scrollback      : atom.config.get 'night-terminal.core.scrollback'
      cols, rows
    }
  ...
  attachListeners: ->
    @ptyProcess.on "night-terminal:data", (data) =>
      @terminal.write data

    @ptyProcess.on "night-terminal:exit", =>
      @destroy() if atom.config.get('night-terminal.toggles.autoClose')
    ...
    @ptyProcess.on "night-terminal:title", (title) =>
      @process = title
    ...
    @terminal.once "open", =>
      ...
      autoRunCommand = atom.config.get('night-terminal.core.autoRunCommand')
  ...
  open: =>
    ...
    @onTransitionEnd =>
      if not @opened
        ...
        @emit "night-terminal:terminal-open"
      else
        @focus()
  ...
  pty: () ->
    if not @opened
      wait = new Promise (resolve, reject) =>
        @emitter.on "night-terminal:terminal-open", () =>
          resolve()
  ...
  ptyPromise: () ->
    new Promise (resolve, reject) =>
      if @ptyProcess?
        @ptyProcess.on "night-terminal:pty", (pty) =>
          resolve(pty)
  ...
  applyStyle: ->
    config = atom.config.get 'night-terminal'
    ...
    @subscriptions.add atom.config.onDidChange 'editor.fontFamily', (event) =>
      ...
      @terminal.element.style.fontFamily = overrideFont or editorFont or defaultFont
    @subscriptions.add atom.config.onDidChange 'night-terminal.style.fontFamily', (event) =>
    ...
    @subscriptions.add atom.config.onDidChange 'editor.fontSize', (event) =>
      ...
      @terminal.element.style.fontSize = "#{overrideFontSize or editorFontSize}px"
      @resizeTerminalToView()
    @subscriptions.add atom.config.onDidChange 'night-terminal.style.fontSize', (event) =>
  ...
  insertSelection: (customText) ->
    ...
    runCommand = atom.config.get('night-terminal.toggles.runInsertedText')
    selectionText = ''
  ...
  getTitle: ->
    @statusIcon.getName() or "night-terminal"
